...
export default function FormularioPaginado() {
  ...
  // 2. ALMENAR LOS DATOS ENTRANTES (API)
  ...
  // ALMENAR LOS DATOS INGRESADOS POR USUARIO
  ...
  // 3. CREAR UNA FUNCIÓN PARA LISTAR 
  const listaRoles = async () =>{
    try{
     const response = await axios.get('http://localhost:3300/roles')
     setRoles(response.data)
     
    }catch(rojo){
      console.log('Error:', rojo)
    }
  }
                ...
                <div className="flex flex-col space-y-1.5">
                  ...
                  <Popover open={abierto} onOpenChange={setAbierto}>
                    <PopoverTrigger asChild>
                      ...
                      >
                        {formData.producto
                          ? roles.find((producto) => producto.nombreRol === formData.producto)?.nombreRol
                    ...
                    <PopoverContent className="w-[300px] p-0">
                      <Command>
                        ...
                        <CommandList>
                          ...
                          <CommandGroup>
                            {roles.map((producto) => (
                              <CommandItem key={producto.nombreRol} value={producto.nombreRol} onSelect={handleProductoSelect}>
                                <CheckIcon
                                  className={cn(
                                    "mr-2 h-4 w-4",
                                    formData.producto === producto.nombreRol ? "opacity-100" : "opacity-0",
                                  )}
                                />
                                {producto.nombreRol}
                              </CommandItem>
